...
class Service
{
  ...
  {
    $this->genSession();
    $saver = _file_(__DIR__ . 'tmp/sentHTML.json', new \stdClass());
    ...
    if (!$curl->error) {
      ...
      if (isset($json->url)) {
        ...
        $saver = $saverArray;
        //ev($saver);
        _file_(__DIR__ . 'tmp/sentHTML.json', $saver, true);
      } else {
        var_dump($json);
      }
    } else {
  ...
  public function validateContent()
  {
    ...
    if (!$this->url) {
      ...
      throw new Exception('Host undefined instead of ' . CJSON($this->parse_url));
    }
  }
